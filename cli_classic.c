...
int main(int argc, char *argv[])
{
	 ...
	 * parsing. */
	if ((options.include_args || options.extract_it) && !options.layoutfile && !options.ifd) {
	...
	if (options.ifd && (flashrom_layout_read_from_ifd(&options.layout, fill_flash, NULL, 0) ||
		...
		goto out_shutdown;
		   ...
		   (flashrom_layout_read_fmap_from_file(&options.layout, fill_flash, options.fmapfile) ||
		    process_include_args(options.layout, options.include_args))) {
		ret = 1;
		goto out_shutdown;
	} else if (!options.ifd && options.fmap &&
		   ((flashrom_layout_read_fmap_from_file(&options.layout, fill_flash, options.filename) &&
		     flashrom_layout_read_fmap_from_rom(&options.layout, fill_flash, 0,
							flashrom_flash_getsize(fill_flash))) ||
		    process_include_args(options.layout, options.include_args))) {
		ret = 1;
		goto out_shutdown;
